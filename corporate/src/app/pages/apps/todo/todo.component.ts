...
import { TodoModel } from 'src/app/store/Todo/todo_model';
import {
  addTodo,
  fetchTodoListData,
  updateTodo,
} from 'src/app/store/Todo/todo_action';
import {
  selectTodoData,
  selectTodoLoading,
} from 'src/app/store/Todo/todo_selector';
...
export class TodoComponent {
  ...
  taskFilter() {
    var status = (
      document.getElementById('choices-select-status') as HTMLInputElement
    ).value;
    if (status) {
  ...
  SortFilter() {
    this.sortField = (
      document.getElementById('choices-select-sortlist') as HTMLInputElement
    ).value;
  ...
  checkUncheckAll(e: any, id: any) {
    ...
    if (checkboxes.checked) {
      status.innerHTML =
        '<span class="badge text-uppercase bg-success-subtle text-success">Completed</span>';
    } else {
      status.innerHTML =
        '<span class="badge text-uppercase bg-secondary-subtle text-secondary">Inprogress</span>';
    }
  }
