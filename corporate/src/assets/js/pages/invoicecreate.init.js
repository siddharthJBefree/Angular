	...
	.addEventListener("change", function () {
		...
		reader.addEventListener(
			...
			function () {
				...
				//sessionStorage.setItem("invoiceLogo", reader.result);
			},
			false
		);
		if (file) {
...
let viewobj;
var invoices_list = sessionStorage.getItem("invoices-list");
...
if (sessionStorage.getItem("invoice_no") === null && sessionStorage.getItem("option") === null) {
	viewobj = '';
	...
	formEvent.addEventListener("submit", function (event) {
		...
		if (formEvent.checkValidity() === false) {
			...
			if ((options == "edit-invoice") && (invoice_no == i_no)) {
				...
				sessionStorage.setItem("new_data_object", JSON.stringify(new_data_object));
			}
